<?php
/**
 * Shared archive listing body (hero + category chips + main-loop card grid +
 * styled pagination + newsletter). Used by category.php / tag.php / archive.php /
 * search.php. Drives the grid from the MAIN query so results are correctly
 * filtered to the current archive.
 *
 * Caller sets before require:
 *   $bx_title  (string) hero H1 (may contain safe markup)
 *   $bx_desc   (string) hero intro paragraph
 *   $bx_active (string) category slug to pre-activate in chips ('' for none)
 *
 * @package BoostronixX
 */
...
$bx_title  = isset( $bx_title ) ? $bx_title : 'Blog';
$bx_desc   = isset( $bx_desc ) ? $bx_desc : '';
$bx_active = isset( $bx_active ) ? $bx_active : '';
$bx_cats   = get_categories( array( 'hide_empty' => true ) );
$bx_all    = get_permalink( get_option( 'page_for_posts' ) );
$bx_all    = $bx_all ? $bx_all : home_url( '/blog/' );
?>
...
<main id="main">
      ...
      <section class="relative pt-24 sm:pt-28 pb-10 grid-lines overflow-hidden">
        <div class="mx-auto max-w-7xl px-4 sm:px-6">
          <nav class="reveal flex items-center gap-2 text-xs text-ink-soft mb-6" aria-label="Breadcrumb">
            ...
            <a href="<?php echo esc_url( $bx_all ); ?>" class="hover:text-accent">Blog</a>
            <iconify-icon icon="lucide:chevron-right" class="text-sm" aria-hidden="true"></iconify-icon>
            <span class="text-ink font-medium" aria-current="page"><?php echo esc_html( wp_strip_all_tags( $bx_title ) ); ?></span>
          </nav>
          <div class="grid lg:grid-cols-12 gap-8 items-end">
            <div class="lg:col-span-8">
              ...
              <h1 class="reveal font-display text-[2.7rem] leading-[0.96] sm:text-7xl tt"><?php echo wp_kses_post( $bx_title ); ?></h1>
            </div>
            <div class="lg:col-span-4">
              <p class="reveal text-ink-soft leading-relaxed" data-delay="1"><?php echo esc_html( $bx_desc ? $bx_desc : 'Practical digital marketing, SEO and branding insights from our Jaipur studio — written by the people who do the work, for people who want to grow.' ); ?></p>
            </div>
          </div>
          <!-- search -->
          <div class="reveal mt-9 max-w-xl" data-delay="2">
            <form role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>">
              ...
              <div class="relative">
                ...
                <input id="blogSearch" name="s" type="search" value="<?php echo esc_attr( get_search_query() ); ?>" placeholder="Search articles…" class="w-full rounded-full border border-line bg-paper py-4 pr-5 text-base outline-none focus:border-accent transition" style="padding-left:3.25rem" autocomplete="off" />
              </div>
            </form>
          </div>
        </div>
      ...
      <!-- FILTER + GRID -->
      <section class="py-10 sm:py-14" aria-label="Articles">
        <div class="mx-auto max-w-7xl px-4 sm:px-6">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-5 mb-10">
            ...
            <div id="chips" class="reveal flex flex-wrap gap-2" role="group" aria-label="Filter by category" data-delay="1">
              <a href="<?php echo esc_url( $bx_all ); ?>" class="chip<?php echo '' === $bx_active ? ' active' : ''; ?> rounded-full border border-line px-4 py-2 text-sm font-medium" data-cat="all" aria-pressed="<?php echo '' === $bx_active ? 'true' : 'false'; ?>">All</a>
              <?php foreach ( $bx_cats as $bx_cat ) : ?>
              <a href="<?php echo esc_url( get_category_link( $bx_cat->term_id ) ); ?>" class="chip<?php echo $bx_active === $bx_cat->slug ? ' active' : ''; ?> rounded-full border border-line px-4 py-2 text-sm font-medium" data-cat="<?php echo esc_attr( $bx_cat->slug ); ?>" aria-pressed="<?php echo $bx_active === $bx_cat->slug ? 'true' : 'false'; ?>"><?php echo esc_html( $bx_cat->name ); ?></a>
              <?php endforeach; ?>
            </div>
          </div>
          ...
          <div id="grid" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            ...
            if ( have_posts() ) :
                ...
                while ( have_posts() ) :
                    the_post();
                    get_template_part( 'template-parts/blog-card', null, array( 'grad' => bx_blog_gradient( $bx_i ) ) );
                    $bx_i++;
                endwhile;
            else :
                ?>
            <p class="text-ink-soft">No articles found.</p>
            <?php endif; ?>
          </div>

          <?php bx_pagination(); ?>

          <!-- empty (client-side search/filter) -->
